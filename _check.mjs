...
// ============================================================================
export const teams = {
  list({ game, region, rank, recruiting } = {}) {
  ...
  async create({ name, game, region, rank, open_roles = [], crest_color }) {
    const u = await auth.getUser(); if (!u) throw new Error('not authed');
    const slug = name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    const team = ok(await sb.from('teams').insert({
      name, slug, owner_id: u.id, game, region, rank,
      recruiting: open_roles.length > 0, open_roles, crest_color
    }).select().single());
...
// ============================================================================
// QUICK MATCH (matchmaking) — minimal version:
//   creates an open "looking for match" scrim and polls for a responder.
//   For production move this to an Edge Function / queue.
// ============================================================================
export const matchmaking = {
  async findMatch({ game, format, server, rank }) {
    // 1) try to grab an existing open scrim near your rank
    const open = await scrims.list({ rank, format, server });
    if (open[0]) return await scrims.accept(open[0].id);
    // 2) else create one and wait for a responder via realtime (handled in UI)
    const u = await auth.getUser();
    return await scrims.create({ team_id: null, game, format, server, rank, note: 'Quick match' });
  }
};

export const COLOSSO = { sb, auth, profiles, players, leaderboard, teams, scrims, lobbies, tournaments, spaces, chat, notifications, matchmaking };
